		...
		m_nMaxPacketSize(MAX_PACKETSIZE)
{
	...
	memset(m_szMyConnIp, 0, sizeof(m_szMyConnIp));
...
void CClientBase::SetLogFileTag(const char* szFileTag)
{
...
string CClientBase::GetTagMyInfo()
{
	...
	if (m_nType == AF_INET)
	{
		...
		{
			snprintf(temp, sizeof(temp), "R%s:%d", m_szMyConnIp, m_sMyConnPort);
		}
	}
	else if (m_nType == AF_UNIX)
	{
...
void CClientBase::SetMyConnIp(const char* szIp)
{
	assert(szIp && strlen(szIp) < sizeof(m_szMyConnIp));
	strcpy(m_szMyConnIp, szIp);
}

string CClientBase::GetMyConnIp()
{
	return m_szMyConnIp;
}

void CClientBase::SetMyConnPort(unsigned short sPort)
{
	m_sMyConnPort = sPort;
}

int CClientBase::GetMyConnPort()
{
	return static_cast<int>(m_sMyConnPort);
}
...
{
	// socket连接成功后，设置连接使用的本地IP和端口
	struct sockaddr_in sa; 
	socklen_t nLen = sizeof(struct sockaddr_in);
	memset(&sa, 0, nLen);  

	if (getsockname(m_nSockfd, (struct sockaddr *)&sa, &nLen) == 0)
	{
		strncpy(m_szMyConnIp, inet_ntoa(sa.sin_addr), IP_LEN);
		m_sMyConnPort = ntohs(sa.sin_port);
	}
...
//			strStatInfo += "  Recv";
//			m_objStreamStatRecv.OnDataStream(pNewBuf->GetUsed(), strStatInfo.c_str());
