...
ENUM_VOL_RATE CPubFuncSystem::ConVGradeNameToId(string strVCLevelType)
{
	...
	else if (strVCLevelType == "AC27.5kV")
	{
	...
	else if (strVCLevelType == "AC210V")
	{
		eVCLevel = EVCR_AC210V;
	}
	else if (strVCLevelType == "AC460V")
	{
		eVCLevel = EVCR_AC460V;
	}
	else if (strVCLevelType == "AC20kV")
	{
		eVCLevel = EVCR_AC20kV;
	}
	return eVCLevel;
}
...
{
	ENUM_FAULTREPORT_TYPE eReportType = EFAT_IMPORT;
	if (strReportType == "关键故障报告")
	{
		eReportType = EFAT_IMPORT;
	}
	else if (strReportType == "其他故障报告")
	{
		eReportType = EFAT_NORMAL;
	}
